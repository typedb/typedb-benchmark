...
abstract class GrandparenthoodAgent<TX: Transaction> protected constructor(client: Client<Session<TX>>, context: Context) :
    Agent<Country, TX>(client, context) {
    override val agentClass = GrandparenthoodAgent::class.java
    ...
    override fun run(session: Session<TX>, region: Country, random: RandomSource): List<Report> {
        ...
        session.reasoningTransaction().use { tx -> matchGrandparents(tx, region, context.today()) }
        return emptyList()
    }

    protected abstract fun matchGrandparents(tx: TX, country: Country, birthDate: LocalDateTime)
}
