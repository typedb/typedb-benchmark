...
public abstract class CitizenshipAgent<TX extends Transaction> extends Agent<Country, TX> {

    protected CitizenshipAgent(Client<?, TX> client, Context context) {
        super(client, context);
    }

    @Override
    protected Class<? extends Agent> agentClass() {
        return CitizenshipAgent.class;
    }
    ...
    protected List<Report> run(Session<TX> session, Country country, RandomSource random) {
        ...
        try (TX tx = session.reasoningTransaction()) {
            ...
            matchCitizenship(tx, country, context.today().minusYears(context.ageOfAdulthood()));
    ...
    protected abstract void matchCitizenship(TX tx, Country country, LocalDateTime today);
}
