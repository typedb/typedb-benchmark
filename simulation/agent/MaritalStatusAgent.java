...
public abstract class MaritalStatusAgent<TX extends Transaction> extends Agent<Country, TX> {

    protected MaritalStatusAgent(Client<?, TX> client, Context context) {
        super(client, context);
    }

    @Override
    protected Class<? extends Agent> agentClass() {
        return MaritalStatusAgent.class;
    }
    ...
    protected List<Report> run(Session<TX> session, Country country, RandomSource random) {
        ...
        try (TX tx = session.reasoningTransaction()) {
            ...
            matchMaritalStatus(tx, country, context.today().minusYears(context.ageOfAdulthood()));
    ...
    protected abstract void matchMaritalStatus(TX tx, Country country, LocalDateTime marriageBirthDate);
}
