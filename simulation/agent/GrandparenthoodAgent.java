...
public abstract class GrandparenthoodAgent<TX extends Transaction> extends Agent<Country, TX> {

    protected GrandparenthoodAgent(Client<?, TX> client, Context context) {
        super(client, context);
    }

    @Override
    protected Class<? extends Agent> agentClass() {
        return GrandparenthoodAgent.class;
    }
    ...
    protected List<Report> run(Session<TX> session, Country country, RandomSource random) {
        ...
        try (TX tx = session.reasoningTransaction()) {
            matchGrandparents(tx, country, context.today());
    ...
    protected abstract void matchGrandparents(TX tx, Country country, LocalDateTime birthDate);
}
