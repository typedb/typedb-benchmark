...
public abstract class NationalityAgent<TX extends Transaction> extends Agent<Country, TX> {

    protected NationalityAgent(Client<?, TX> client, Context context) {
        super(client, context);
    }

    @Override
    protected Class<? extends Agent> agentClass() {
        return NationalityAgent.class;
    }
    ...
    protected List<Report> run(Session<TX> session, Country country, RandomSource random) {
        ...
        try (TX tx = session.reasoningTransaction()) {
            matchNationalities(tx, country, context.today());
    ...
    protected abstract void matchNationalities(TX tx, Country country, LocalDateTime today);
}
