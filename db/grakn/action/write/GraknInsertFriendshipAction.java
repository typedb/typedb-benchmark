...
public class GraknInsertFriendshipAction extends InsertFriendshipAction<GraknDbOperationController.TransactionalDbOperation, ConceptMap> {

    public GraknInsertFriendshipAction(TransactionDbOperationController<GraknTransaction>.TransactionalDbOperation dbOperation, LocalDateTime today, String friend1Email, String friend2Email) {
    ...
    public ConceptMap run() {
        ...
        return Action.optionalSingleResult(dbOperation.tx().execute(insertFriendshipQuery));
    }
    ...
    protected HashMap<ComparableField, Object> outputForReport(ConceptMap answer) {
        return new HashMap<ComparableField, Object>() {
            {
                put(InsertFriendshipActionField.FRIEND1_EMAIL, dbOperation.tx().getOnlyAttributeOfThing(answer, "p1", EMAIL));
                put(InsertFriendshipActionField.FRIEND2_EMAIL, dbOperation.tx().getOnlyAttributeOfThing(answer, "p2", EMAIL));
                put(InsertFriendshipActionField.START_DATE, dbOperation.tx().getOnlyAttributeOfThing(answer, FRIENDSHIP, START_DATE));
            }
        };
